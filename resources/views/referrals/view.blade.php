<div x-show="openViewReferralModal_{{ $referral->id }}"
     x-transition
    ...
    <div class="bg-white rounded-xl shadow-lg max-w-5xl w-full p-6 relative overflow-y-auto max-h-[90vh] border border-gray-300">
        ...
        <!-- Close Button -->
        <button @click="openViewReferralModal_{{ $referral->id }} = false"
                class="absolute top-2 right-2 text-gray-500 hover:text-red-600 text-2xl font-bold transition">
            &times;
        </button>

        <!-- Title -->
        <h3 class="text-2xl font-bold mb-6 text-center text-red-700 border-b pb-2">Referral Details</h3>
        ...
        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200 mb-6">
            <div class="grid w-full text-sm text-gray-800">
                <div class="flex flex-col bg-gray-100 dark:bg-gray-800 p-3 rounded shadow-sm">
                    ...
                    <span class="text-base text-gray-800 dark:text-gray-300 font-medium">{{ $referral->remarks ?? 'No remarks provided.' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200">
            ...
            @if($referral->images && $referral->images->count() > 0)
